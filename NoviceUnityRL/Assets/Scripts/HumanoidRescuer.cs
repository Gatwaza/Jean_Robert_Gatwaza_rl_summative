// HumanoidRescuer.cs (v4 — proper kneeling, arms reach patient chest, compression pump)
// Both knees on the ground. Arms lock straight during compression and visibly
// pump down onto patient.ChestWorldPosition. IK is rotation-only — no floating.
...
public class HumanoidRescuer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (_ikActive) DriveArmsToward(_leftTarget, _rightTarget);
    }
    ...
    // ── Compression: wrists driven to patient chest with pump depth ───────────
    void BeginCompression()
    {
        _compressing = true; _ikActive = false; _compPhase = 0f;
        // Straighten forearms and wrists for CPR posture
        ...
        if (_body.rightHand    != null) _body.rightHand.localEulerAngles    = new Vector3(-8f,0f,0f);
    }
    ...
    void DoCompressionIK()
    {
        ...
        float pump = Mathf.Abs(Mathf.Sin(_compPhase));   // 0 = up, 1 = full depth

        // Lean forward on downstroke — weight transfer
        if (_body.spine != null)
            _body.spine.localEulerAngles = new Vector3(Mathf.Lerp(20f, 42f, pump), 0f, 0f);

        // Drive wrists to patient chest + downward pump offset
        Vector3 chest  = GetChestPos();
        Vector3 offset = -transform.up * (pump * 0.065f);
        Vector3 target = chest + offset;
    ...
    {
        _ikActive = true;
        _leftTarget  = GetPatientShoulder(true)  + Vector3.up * 0.05f;
        _rightTarget = GetPatientShoulder(false) + Vector3.up * 0.05f;
        yield return new WaitForSeconds(0.5f);
        for (int i = 0; i < 2; i++)
        {
            _leftTarget  += Vector3.up * 0.07f; _rightTarget += Vector3.up * 0.07f;
    ...
    IEnumerator AnimOpenAirway()
    {
        ...
        _leftTarget  = hp + transform.forward*0.10f;
        _rightTarget = hp - transform.forward*0.10f;
    ...
    IEnumerator AnimDefibrillate()
    {
        ...
        _ikActive = true;
        _rightTarget = aed + Vector3.up*0.5f;
        _leftTarget  = RestLeft();
    ...
    IEnumerator AnimTiltHead()
    {
        _ikActive = true;
        Vector3 hp = GetPatientHead();
        _leftTarget  = hp + transform.forward * 0.09f;
        _rightTarget = hp - transform.forward * 0.09f;
    ...
    // ── IK — rotation-only, no hand.position assignment ──────────────────────
    void DriveArmsToward(Vector3 leftWorld, Vector3 rightWorld)
    {
        SolveTwoBoneIK(_body.leftUpperArm,  _body.leftForearm,  _body.leftWrist,  leftWorld,  true);
        SolveTwoBoneIK(_body.rightUpperArm, _body.rightForearm, _body.rightWrist, rightWorld, false);
    }
                        ...
                        Vector3 targetWorld, bool left)
    {
        ...
        if (toTarget.sqrMagnitude > 0.0001f)
        {
            ...
            upper.rotation = Quaternion.Slerp(upper.rotation,
                Quaternion.LookRotation(fwd, up), Time.deltaTime*ikSpeed);
        }

        float cosA = (upperLen*upperLen + dist*dist - midLen*midLen)
        ...
        mid.localRotation = Quaternion.Slerp(mid.localRotation,
            Quaternion.AngleAxis(angle, axis), Time.deltaTime*ikSpeed);
    }
    ...
    IEnumerator ReturnToRest()
    {
        ...
        while (t < 1f)
        {
            ...
            _body.leftWrist.localRotation     = Quaternion.Slerp(lwS,Quaternion.identity,s);
            _body.rightWrist.localRotation    = Quaternion.Slerp(rwS,Quaternion.identity,s);
            yield return null;
        }
    }

    // ── Kneeling pose — BOTH knees on ground ─────────────────────────────────
    void SetKneeling()
    {
        if (_body == null) return;
        // Hips lower than standing, both thighs fold forward (kneeling)
        if (_body.hips       != null) _body.hips.localPosition      = new Vector3(0f, 0.50f, 0f);
        // Thighs angle forward (−90° = pointing down = kneeling)
        if (_body.leftThigh  != null) _body.leftThigh.localEulerAngles  = new Vector3(-88f,0f, 6f);
        if (_body.rightThigh != null) _body.rightThigh.localEulerAngles = new Vector3(-88f,0f,-6f);
        // Shins fold back to rest on ground
        if (_body.leftShin   != null) _body.leftShin.localEulerAngles   = new Vector3( 88f,0f, 0f);
        if (_body.rightShin  != null) _body.rightShin.localEulerAngles  = new Vector3( 88f,0f, 0f);
        // Feet flat (dorsiflexed for kneeling)
        if (_body.leftFoot   != null) _body.leftFoot.localEulerAngles   = new Vector3(-20f,0f,0f);
        if (_body.rightFoot  != null) _body.rightFoot.localEulerAngles  = new Vector3(-20f,0f,0f);
        if (_body.spine      != null) _body.spine.localEulerAngles      = new Vector3( 15f,0f, 0f);
    }
    ...
    Vector3 GetPatientShoulder(bool left) =>
        patient != null ? patient.transform.position + new Vector3(left?0.20f:-0.20f, 0.22f, 0.28f)
